...
| `FireworksBackend`  | Provider runtime             | **Native GBNF** (`type: grammar`)            | The cleanest "logit-tier on a hosted API" backend — citeformer's `cite-id` GBNF rule is dropped in unchanged via Fireworks's [grammar mode](https://docs.fireworks.ai/structured-responses/structured-output-grammar-based). Same constraint that masks logits inside `HFBackend`, just running on Fireworks's GPUs. |
